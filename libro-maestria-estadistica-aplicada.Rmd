...
```{r scatter-dur-cut, fig.width=9, fig.height=4.5}
datos %>%
  ggplot(aes(x = duracion_proceso, y = costo_unitario_total)) +
  geom_point(alpha = 0.4, color = "#1E90FF") +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "red") +
  labs(x = "Duración del proceso (horas)",
       y = "Costo unitario  (UM/unidad)") +
  theme_bw() +
  facet_grid(. ~ "Dispersión entre duracion_proceso y costo_unitario_total")
```



```{r scatter-cantidad-cut, fig.width=9, fig.height=4.5}
cantidad_total_insumo == cantidad_insumo_1 + cantidad_insumo_2
datos %>%
  ggplot(aes(x = cantidad_total_insumo, y = costo_unitario_total)) +
  geom_point(alpha = 0.4, color = "#1E90FF") +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "red") +
  labs(x = "Cantidad total de insumo",
       y = "Costo unitario  (UM/unidad)") +
  theme_bw() +
  facet_grid(. ~ "Dispersión entre cantidad_total_insumo y costo_unitario_total")
```
