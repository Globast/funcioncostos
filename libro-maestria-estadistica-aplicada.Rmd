...
modproceso_media  <- cc_modproceso$media
modproceso_ds     <- cc_modproceso$ds
modproceso_med    <- cc_modproceso$`50%`
modproceso_min    <- cc_modproceso$minimo
modproceso_max    <- cc_modproceso$maximo
modproceso_iqr    <- cc_modproceso$IQR
modproceso_q1     <- cc_modproceso$Q1
modproceso_q3     <- cc_modproceso$Q3
modproceso_cv     <- modproceso_ds / modproceso_media * 100

  knitr::kable(cc_modproceso , caption = "Resumen Bivariado costos de transformación por modalidad de proceso",
                 format = "html",
             table.attr = "class='table'") %>%
  kableExtra::kable_styling(full_width = FALSE) 
...
En transformación: se observa un gradiente claro X (`r modproceso_media[1]`) < Y (`r modproceso_media[2]`) < Z (`r modproceso_media[3]`), confirmado por la mediana. Pero la dispersión cambia entre modalidades: X tiene desviación estándar de 17.9 (proceso volátil), Y tiene `r modproceso_ds[2]` (proceso controlado), Z tiene `r modproceso_ds[3]` pero con IQR contenido (alto y consistente).

En insumo: el patrón es más suave (X: `r modproceso_media[1]`, Y: `r modproceso_media[2]`, Z: `r modproceso_media[3]`), y la mediana de Z (`r modproceso_med[3]`) está muy cerca de la de Y. El sobrecosto de Z proviene principalmente del componente de transformación, no del de insumos.
...
Limitación crítica. El n de Z es de `r cc_modproceso$n[3]` órdenes. Cualquier conclusión cuantitativa sobre Z tiene intervalos de confianza muy amplios. Z aparece aquí como observación cualitativa, no como base para inferencia.
...
ci_modproceso_media  <- ci_modproceso$media
ci_modproceso_ds     <- ci_modproceso$ds
ci_modproceso_med    <- ci_modproceso$`50%`
ci_modproceso_min    <- ci_modproceso$minimo
ci_modproceso_max    <- ci_modproceso$maximo
ci_modproceso_iqr    <- ci_modproceso$IQR
modproceso_q1     <- cc_modproceso$Q1
modproceso_q3     <- cc_modproceso$Q3
modproceso_cv     <- modproceso_ds / modproceso_media * 100
