...
Relación casi perfecta entre el costo de transformación y el costo unitario total (`r round(matriz_correlacion[1,3], 3)`). Este resultado no es sorprendente, ambas variables contienen prácticamente la misma información, por lo que analizarlas por separado aporta poco valor adicional.
...
En cuanto a las variables operativas, las correlaciones son altas y consistentes entre volumen de lote, duración del proceso y cantidad de insumo 1 (todas por encima de 0.6). Este patrón refleja una estructura productiva coherente: a mayor volumen, mayor duración y mayor consumo de insumos. Es decir, el sistema escala de forma integrada.
...
```{r bar-categoricas, fig.width=12, fig.height=16}
library(dplyr)
library(ggplot2)
