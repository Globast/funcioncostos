...
datos <- datos %>%
  mutate(
    comb_insumos = if_else(
      num_insumos == 1,
      insumo_1,
      paste(pmin(insumo_1, insumo_2),
...
```{r box-insumo-comb}
datos %>%
  ggplot(aes(x = comb_insumos, y = costo_transformacion)) +
...
datos %>%
  ggplot(aes(x = comb_insumos, y = costo_insumo)) +
