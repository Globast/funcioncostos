...
matriz_resaltada <- matriz_spearman

for(i in 1:nrow(matriz_resaltada)) {
  for(j in 1:ncol(matriz_resaltada)) {
    valor <- matriz_spearman[i, j]
    
    matriz_resaltada[i, j] <- if(abs(valor) > 0.5) {
      kableExtra::cell_spec(valor, bold = TRUE, color = "white", background = "darkred")
    } else {
      as.character(valor)
    }
  }
