...
Dado que las tres categorías causales convergen en un rasgo común —ausencia de costo de insumo asignable al producto conforme— la variable `costo_insumo == 0` captura exactamente las `r n_casos_cero` órdenes a eliminar y se adopta como criterio único de filtrado. Este criterio es preferible a un filtro compuesto porque es más parsimonioso y tiene una interpretación operativa directa: toda orden de producción válida debe registrar un costo de insumo mayor que cero, independientemente de si el cero proviene de error de digitación, orden no ejecutada o producción no conforme.
