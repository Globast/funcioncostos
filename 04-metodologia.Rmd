...
**Tratamiento de valores atípicos.** El presente capítulo aborda únicamente la limpieza de errores de digitación confirmados a priori con el área de operaciones, correspondientes a registros con `num_insumos = 0`. La detección e interpretación de valores atípicos estadísticos se realiza en el capítulo de análisis exploratorio, con base en boxplots y estadísticos robustos (rango intercuartílico). La decisión final sobre retención, eliminación o análisis separado de dichos valores se toma tras consulta con el área de operaciones, que permite distinguir errores de registro, eventos excepcionales y variabilidad legítima del proceso.
...
La inspección de la estructura revela que varias variables están almacenadas con tipos que no corresponden a su naturaleza analítica. La variable `periodo` viene como entero `YYYYMM`; en análisis posteriores se podrá convertir a fecha para análisis de temporalidad, pero en el presente estudio se conserva como texto, dado que no se realizarán análisis de series de tiempo. La variable `num_insumos`, restringida a los valores 0, 1 y 2, se trata como factor ordenado. Las variables `tipo_equipo`, `categoria_producto`, `equipo_id`, `modalidad_proceso`, `insumo_1` e `insumo_2` se convierten a factor para los análisis inferenciales posteriores. La variable `insumo_2` requiere además una recodificación explícita de los valores faltantes a la categoría `sin_segundo_insumo`, dado que el `NA` no representa un dato perdido sino una característica estructural de la orden (cuando `num_insumos < 2`).
...
Después de la eliminación, el dataset analítico queda conformado por las órdenes con al menos un insumo registrado. La función `droplevels()` elimina el nivel `0` del factor ordenado para evitar que aparezca vacío en tablas y gráficos posteriores.

### Advertencias sobre inconsistencias estructurales

Antes de proceder al análisis exploratorio se documentan dos aspectos estructurales del dataset que, si no se declaran explícitamente, pueden inducir interpretaciones erróneas.
